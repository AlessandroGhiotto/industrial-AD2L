...
from adl_lib.efficient_ad import EfficientAD
import os

def train_efficient_ad(model, train_loader, epochs=20, lr=0.0001, device="cuda"):
    model.to(device)
    # Train both student and autoencoder
    optimizer = Adam(list(model.student.parameters()) + list(model.autoencoder.parameters()), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            # MSE loss between teacher and student, and teacher and autoencoder
            loss_st = torch.mean((teacher_features - student_features) ** 2)
            loss_ae = torch.mean((teacher_features - ae_features) ** 2)
...
def main():
    ...
    print(f"Loading EfficientAD Model on {device}")
    model = EfficientAD()
    ...
    train_efficient_ad(model, good_loader, epochs=15, lr=0.0001, device=device)
    ...
    save_results_to_pdf(all_class_results, "artifacts/efficient_ad_predictions.pdf")
    print("Saved PDF to artifacts/efficient_ad_predictions.pdf")
